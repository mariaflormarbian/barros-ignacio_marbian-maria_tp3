...
function createGallery(req, res) {
  ...
  GalleryService.addToGallery(idProject, gallery)
    ...
    .catch((err) => {
      res
        .status(500)
        .json({ message: "Error al guardar la galería", error: err });
    });
}
...
function findGalleryByProject(req, res) {
  ...
  GalleryService.findGalleryByProject(idProject)
    ...
    .catch((err) => {
      res
        .status(500)
        .json({ message: "Error al obtener la galería", error: err });
    });
}
...
function deleteImageById(req, res) {
  ...
  GalleryService.deleteImageById(idProject, idImage)
    ...
    .catch((err) => {
      res
        .status(500)
        .json({ message: "Error al eliminar la imagen", error: err });
    });
}

export { createGallery, findGalleryByProject, deleteImageById };
